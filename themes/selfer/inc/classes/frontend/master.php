...
if( ! class_exists( 'Selfer_Master' ) ) :
    class Selfer_Master {
        ...
        public function page_title( $title, $sep = ' | ' ) {
            ...
            $filtered_title .= ( 2 <= $paged || 2 <= $page ) ? $sep . sprintf( __( 'Page %s', 'selfer' ), max( $paged, $page ) ) : '';
        ...
        public function excerpt_length( $length ) {
            $length = selfer_get_options(array('excerpt_length','25'));
            return $length;
        }
        ...
        public function excerpt_more( $more ) { 
            return '<a href="' . get_permalink() . '" class="more-link bg-blue-violet color-white">' . esc_html__( 'Read More', 'selfer' ) . '<i class="fa fa-arrow-right"></i></a>';
        }
