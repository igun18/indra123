 ...
 * @since 1.0
 */
function selfer_get_standard_fonts() {
	return apply_filters(
		'selferstandard_fonts_array', array(
 ...
 * @since 1.0
 */
function selfer_enqueue_google_font( $font ) {
	...
	$subsets     = '';
	$get_subsets = selfer_get_options( array('font_subsets', array( 'latin' ) ) );
	...
	// Weights
	$weights = apply_filters( 'selfer_font_weights', array( '300', '400', '500', '700', '800' ) );
	...
	// Enqueue style
	wp_enqueue_style( 'selfer-google-font-' . $handle, $url, array(), false );
}

function selfer_enqueue_google_font_url( $font ) {
	// Get list of all Google Fonts
	$google_fonts = selfer_get_google_fonts_array();
