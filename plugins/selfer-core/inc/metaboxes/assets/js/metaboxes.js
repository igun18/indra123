...
$(document).ready( function () {
	var minHeight = $('ul.selfer_metabox_tabs').height();
	
	function set_tab_height() {
		var height = $('div.selfer_metabox').height();
		$('ul.selfer_metabox_tabs').height(
			height > minHeight ? ( height + 40 ): minHeight	
		);
	}

	$(".selfer_metabox_tabs a").click ( function ( e ) {
		e.preventDefault();

		$(".selfer_metabox_tabs li").removeClass('active');
		$(this).closest( 'li' ).addClass('active');

		$('.selfer_metabox_tab').hide();

		var tabid = $(this).attr( 'href' );
		tabid = tabid.replace( '#', '' );

		$('#selfer_tab_' + tabid + '.selfer_metabox_tab').fadeIn( 300,
				function ( ) {
					set_tab_height();
				}
		);
	...
	$(".selfer_metabox_tabs li.active a").trigger('click');

	$('.selfer_metabox input.selfer_upload_button').click( function(e) {
		e.preventDefault();
		var self = $(this);
	...
	var custom_uploader;
	$('.selfer_multiple_images').each( function() {
		var multiple_image_wrapper = $(this);
		var id = multiple_image_wrapper.data('formid');
		$(this).find('.selfer_multiple_upload_button').click( function(e) {
			e.preventDefault();
		    ...
		    custom_uploader.on('select', function() {
		    	...
		    	var upload_container = multiple_image_wrapper.find('.selfer_uploaded_images');
		        selection.map( function( attachment ) {
			        attachment = attachment.toJSON();
			        var img_html = '';
			        img_html += '<div class="image">';
			        img_html += ( '<img src="' + attachment.sizes['thumbnail'].url + '">' );
			        img_html += ( '<input type="hidden" id="selfer_' + id + '" name="selfer_' + id + '[]" value="' + attachment.id + '">' );
	...
	$(document).on( 'click', '.selfer_uploaded_images .remove', function(){
		var img_container = $(this).closest('.image');
		img_container.remove();
	} );

	$('.selfer-metabox-color-picker').wpColorPicker();

	/* Metabox option group toggle open/close */
	$('.selfer-metabox-subsection .subsection-title').on( 'click', function() {
		var $section = $(this).closest( '.selfer-metabox-subsection' );
	...
	var option_prefix = 'selfer_';
	function metabox_check_dependency() {
		$( '.selfer_metabox' ).find( 'div[data-dep-option]' ).each( function() {
			var $this = $(this);
	...
	$( '.selfer-meta-value' ).on( 'change', function() {
		metabox_check_dependency();
	} );
});
})( jQuery );