 ...
 * @since 1.0
 */
class Selfer_Layer_Slider_Widget extends Widget_Base {

	public function get_name() {
		return 'slefer-layerslider';
	}

	public function get_title() {
		return esc_html__( 'Layer Slider', 'selfer-core' );
	}
	...
	protected function register_controls() {
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', '%', 'em' ],
				'selectors' => [
					'{{WRAPPER}} .selfer-layer-slider' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);		
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', '%', 'em' ],
				'selectors' => [
					'{{WRAPPER}} .selfer-layer-slider' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
	...
	protected function get_layer_slider_list() {
		global $wpdb;
        $slides_array = array();
        $slides_array[0] = esc_html__( 'Select a slider', 'selfer-core' );
        ...
        if( isset( $slides ) && $slides ) {
        	foreach ( $slides as $key => $val ) {
        		$slides_array[$key] = esc_html__( 'LayerSlider #', 'selfer-core' ) . ($key);
        	}
        }
...
if (is_plugin_active('LayerSlider/layerslider.php')){
	Plugin::instance()->widgets_manager->register_widget_type( new Selfer_Layer_Slider_Widget() );
}